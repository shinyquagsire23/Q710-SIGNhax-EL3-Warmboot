...
def make_request(cmd, args=[], body=b'', test=0):
...
def make_exec_request(shell_command, rawshell):
    ...
    if (shell_command == 'signtest'):
        sig = open('signtest.bin', 'rb').read()
        return make_request(b'SIGN', body=sig)
    elif (shell_command == 'signtest-gen'):
        sig = open('signtest-gen.bin', 'rb').read()
        return make_request(b'SIGN', body=sig)
    elif (shell_command == 'writetest-gen'):
        zlib_comp = 0
        fd = 0x48
        offset = 0x0d8000 # 0x0d8000 0x0da000
        unk = 0
        flags = 0x00210030 | (zlib_comp << 3)
        data = open('raw_resources_a_mod.image', 'rb').read()
        return make_request(b'WRTE', args=[struct.pack("<L", fd), struct.pack("<L", offset), struct.pack("<L", unk), struct.pack("<L", flags)], body=data)
    elif (shell_command == 'writetest-gen-2'):
        zlib_comp = 0
        fd = 0x48
        offset = 0x0da000 # 0x0d8000 0x0da000
        unk = 0
        flags = 0x00210030 | (zlib_comp << 3)
        data = open('raw_resources_a_mod.image', 'rb').read()
        return make_request(b'WRTE', args=[struct.pack("<L", fd), struct.pack("<L", offset), struct.pack("<L", unk), struct.pack("<L", flags)], body=data)
    elif (shell_command == 'closetest'):
        fd = 0x48
        return make_request(b'CLSE', args=[struct.pack("<L", fd), b'\0\0\0\0', b'\0\0\0\0', b'\0\0\0\0'])
    elif (shell_command == 'opentest'):
        return make_request(b'OPEN', args=[struct.pack("<L", 0), b'\0\0\0\0', b'\0\0\0\0', b'\0\0\0\0'])
#    elif (shell_command == 'opentest'):
#        return make_request(b'OPEN', 
    else:
...
class USBCommunication(Communication):
    ...
    def write(self, data):
        ...
        len_to_write = len(data)
        written = 0
        while (len_to_write > 0):
            to_write = 0x4000
            if (len_to_write < 0x4000):
                to_write = len_to_write
            self.usbdev.write(self.ep_out, data[written:written+to_write])
            written += to_write
            len_to_write -= to_write
    def close(self):
...
def try_hello(comm):
    ...
    hello_request = make_request(b'HELO', args=[b'\x13\0\0\1'])
